...
public class SuggestionModelRequestService {
    ...
    public final ResponseEntity<String> createSuggestion(final String auth, final String body) throws LibraryException {
        ...
        if(suggestionRequestModel.getNameOfBook() == null || suggestionRequestModel.getIsbn() == null || suggestionRequestModel.getSuggestionStatusType() == null)
            throw new LibraryException(ErrorType.BAD_REQUEST);

        return this.suggestionModelService.createSuggestion(userModel, suggestionRequestModel.getNameOfBook(), suggestionRequestModel.getIsbn()).toResponse().toResponseEntity(HttpStatus.OK);
    }
